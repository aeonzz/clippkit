...
<Callout title="COMING SOON" type="warn">
  Installation via `npx shadcnui install clipkittt@toast-card` isn't quite ready yet.
...
<Steps>
  <Step>
    Create a new file, for example, at `src/components/toast-card.tsx` (or your
    preferred location) and paste the code from
    `apps/docs/registry/default/ui/toast-card.tsx` into it.
  </Step>
