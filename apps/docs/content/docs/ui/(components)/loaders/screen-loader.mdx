...
<Callout title="COMING SOON" type="warn">
  Installation via `npx shadcnui install clipkittt@screen-loader` isn't quite ready yet.
  For now, please follow the manual installation instructions.

Eventually users will be able to run the following command:

      ```package-install
    npx shadcn@latest add "https://clippkit/r/screen-loader"
    ```
...
<Steps>
  <Step>
    ...
    component code in the `apps/docs/registry/default/ui/screen-loader.tsx` file or
